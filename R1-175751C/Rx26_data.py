...
for i in month:
    ...
    for j in range(1,32,1):
        #print(j)
        path = ["RxData/2009" + str(i) + "/2009" + str(i) + str(j).zfill(2) + "/192.168.100.11_csv.log"]
        #print(path)
        for k in path:
            #print(k)
            try:
            ...
            file_time = file[["time"]]
            file = file[[" MX_RX_LEVEL"]]
        
        
            try:
                f = lambda x: (x + 256)/2 - 121 if x < 0 else x/2 - 121
                file_Rx = file[" MX_RX_LEVEL"].map(f)
        ...
        #結果をfileに書き込む
            df.to_csv("data_Rx26.csv",mode="a",header=False)
